...
namespace PdfReport.PDFGenerator
{
    ...
    {
        public Reporting.SRScoveragestats srscoveragestats = new Reporting.SRScoveragestats();
        ...
        void LAMBDALINK(object sender, EventArgs e)
        {
            try
            {
                ...
                Reporting.SRSTargetstats temptarget = new Reporting.SRSTargetstats();
                ...
                if (t2list.SelectedItem.ToString() != "NA")
                {
                   // MessageBox.Show("t2 grab");
                    srscoveragestats.Targets.Add(new Reporting.SRSTargetstats { TargetNAME = t2list.SelectedItem.ToString(), Dose = Convert.ToDouble(DBox2.Text) });
                }

                if (t3list.SelectedItem.ToString() != "NA")
                {
                    srscoveragestats.Targets.Add(new Reporting.SRSTargetstats { TargetNAME = t3list.SelectedItem.ToString(), Dose = Convert.ToDouble(DBox3.Text) });
                }

                if (t4list.SelectedItem.ToString() != "NA")
                {
                    srscoveragestats.Targets.Add(new Reporting.SRSTargetstats { TargetNAME = t4list.SelectedItem.ToString(), Dose = Convert.ToDouble(DBox4.Text) });
                }

                if (t5list.SelectedItem.ToString() != "NA")
                {
                    srscoveragestats.Targets.Add(new Reporting.SRSTargetstats { TargetNAME = t5list.SelectedItem.ToString(), Dose = Convert.ToDouble(DBox5.Text) });
                }
            }
            catch(Exception ex)
            {
