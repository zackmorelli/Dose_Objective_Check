...
namespace PdfReport.Reporting.MigraDoc.Internal
{
    class SRSstats
    {
        public void Add(Section section, ReportData data)
        {
            ...
            ROI.ROI BodyMaxDose = data.PROI.Where(ob => ob.ROIName.Equals("Body Max Point Dose (Global Max)")).First();
            ...
            int count = 0;
            foreach (Reporting.SRSTargetstats targetstat in data.srsstats.Targets)
            {
